...
export const MIN_REMINDER_LEAD_HOURS = 24;
...
export function validateReminderHours(values: number[]): number[] {
  if (values.some((value) => !Number.isFinite(value) || value < MIN_REMINDER_LEAD_HOURS)) {
    throw new Error(`Each scheduled reminder must be at least ${MIN_REMINDER_LEAD_HOURS} hours before the task. Enter hours such as 48, 24.`);
  }
