...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  },
  {
    ...
    sections: [
      { heading: 'Create a task', text: 'Provide a title and Eastern Time (ET) start date/time. Optionally add an event, description, end time, location, volunteer count, recurrence, and up to two comma-separated reminder hours, such as 48, 24. Every reminder must be at least 24 hours before the task, and two reminders must be at least 24 hours apart. Invalid reminder timing is not saved and displays a correction message. Every task is open for volunteer self-signup until full.' },
  ...
  },
  {
    ...
    sections: [
      { heading: 'Timing', text: 'Each task can store up to two reminder times, expressed as hours before its Eastern Time (ET) start. Every reminder must be at least 24 hours before the task, and two reminders must be at least 24 hours apart. Google Cloud Scheduler runs the sender hourly at 7 minutes past the hour. A two-hour recovery window catches slightly delayed runs, and duplicate protection sends each configured reminder only once.' },
