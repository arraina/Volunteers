...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  },
  {
    ...
    sections: [
      { heading: 'Create a task', text: 'Provide a title and a current or future Eastern Time (ET) start date/time; past task times cannot be created or selected when rescheduling. Optionally add an event, description, end time, location, volunteer count, recurrence, and up to two comma-separated reminder hours. One reminder may be any positive number of hours before the task; two reminders must be at least 24 hours apart, such as 48 and 24. Invalid timing is not saved and displays a correction message. Every task is open for volunteer self-signup until full.' },
